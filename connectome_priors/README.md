...
`generate_ncap_segment_priors(num_segments)` in `NMAP/connectome_priors/swimmer_priors.py` now:
...
`NMAP/swimmer/models/tonic_ncap.py` now:
...
`NMAP/swimmer/training/custom_tonic_agent.py` now:
...
`NMAP/connectome_priors/demo_swimmer_priors.ipynb` now:
...
- smoke tests updated for scalar sparse prior API:
  - `NMAP/connectome_priors/tests/test_priors_smoke.py`
- package import surface now degrades gracefully when torch-backed wrappers are unavailable:
  - `NMAP/connectome_priors/__init__.py` keeps sparse-prior utilities importable without forcing swimmer runtime deps
...
- manual smoke assertions equivalent to `NMAP/connectome_priors/tests/test_priors_smoke.py` passed in local `nmap` environment.
...
3. If required for review scope, remove remaining legacy wrapper API in `NMAP/connectome_priors/swimmer.py` to make package surface sparse-only.
